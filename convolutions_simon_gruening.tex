...
&= (f \ast (g \ast  h ))(x) \\
\end{align*}
Distributivity follow from the additive properties of the integral:
\begin{align*}
...
\begin{proof}
First we notice that the case of $p=1$ follows from (Lemma 3.1). Similarly we may rid ourselves of the easy case $p=\infty$ by again applying the triangle inequality and noticing that the $ess.sup$ always minimizes our norm:
 (WTF ZZZZZZZZZZZZZZZ)
\begin{align*}
\norm{(f \ast g)}_{L^\infty} &\leq \norm{(f \ast g)}_{L^1} \\
&= \abs{\int_G f(y)g(y^{-1}x) d\lambda(y)} \\
